...
class Bin2ConstexprCharWriter : public bx::WriterI
{
public:
	Bin2ConstexprCharWriter(bx::WriterI* _writer, const char* _name) : m_writer(_writer), m_name(_name)
	{
	}

	virtual ~Bin2ConstexprCharWriter()
	{
	}
...
#define HEX_DUMP_FORMAT "%-" BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "." BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "s"
		...
		bx::writePrintf(m_writer, "constexpr const char %s[%d] =\n", m_name.c_str(), m_buffer.size());
...
void help(const char* _error = NULL)
{
	...
	fprintf(stderr,
			"bin2constexprchar, binary to C const char*\n"
			"Copyright 2017 Christian Helmich. All rights reserved.\n"
			"Based on work by Branimir Karadzic.\n"
			"License: https://github.com/bkaradzic/bx#license-bsd-2-clause\n\n");

	fprintf(stderr,
			"Usage: bin2constexprchar -f <in> -o <out> -n <name>\n"
...
int main(int _argc, const char* _argv[])
{
	...
	if (bx::open(&fr, filePath))
	{
		...
		{
			Bin2ConstexprCharWriter writer(&fw, name);
