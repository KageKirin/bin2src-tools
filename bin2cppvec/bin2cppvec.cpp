...
class Bin2CppVecWriter : public bx::WriterI
{
public:
	Bin2CppVecWriter(bx::WriterI* _writer, const char* _name) : m_writer(_writer), m_name(_name)
	{
	}

	virtual ~Bin2CppVecWriter()
	{
	}
...
#define HEX_DUMP_FORMAT "%-" BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "." BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "s"
		...
		bx::writePrintf(m_writer, "extern const std::vector<uint8_t> %s; // for linker happiness\n", m_name.c_str());
		bx::writePrintf(m_writer, "const std::vector<uint8_t> %s{\n", m_name.c_str());
...
int main(int _argc, const char* _argv[])
{
	...
	if (bx::open(&fr, filePath))
	{
		...
		{
			Bin2CppVecWriter writer(&fw, name);
