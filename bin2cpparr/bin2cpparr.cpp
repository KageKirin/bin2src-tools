...
class Bin2CppArrWriter : public bx::WriterI
{
public:
	Bin2CppArrWriter(bx::WriterI* _writer, const char* _name) : m_writer(_writer), m_name(_name)
	{
	}

	virtual ~Bin2CppArrWriter()
	{
	}
...
#define HEX_DUMP_FORMAT "%-" BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "." BX_STRINGIZE(HEX_DUMP_SPACE_WIDTH) "s"
		...
		bx::writePrintf(m_writer, "#include <array>\n\n");
		bx::writePrintf(m_writer, "extern const std::array<uint8_t, %d> %s; // for linker happiness\n", size, m_name.c_str());
		bx::writePrintf(m_writer, "const std::array<uint8_t, %d> %s{\n", size, m_name.c_str());
...
void help(const char* _error = NULL)
{
	...
	fprintf(stderr,
			"bin2cpparr, binary to C++ std::array\n"
			"Copyright 2017 Christian Helmich. All rights reserved.\n"
			"Based on work by Branimir Karadzic.\n"
			"License: https://github.com/bkaradzic/bx#license-bsd-2-clause\n\n");

	fprintf(stderr,
			"Usage: bin2cpparr -f <in> -o <out> -n <name>\n"
...
int main(int _argc, const char* _argv[])
{
	...
	if (bx::open(&fr, filePath))
	{
		...
		{
			Bin2CppArrWriter writer(&fw, name);
